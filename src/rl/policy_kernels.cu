...
//     CUDA_OK(cudaMemcpy(h_logits.data(), d_probs_, sizeof(float) * h_probs.size(), cudaMemcpyDeviceToHost));
// }  
...
/*
Linear softmax policy:  pi(a|s) = softmax(W^T s),  W in [D, A]

Forward pass:
  logits[B, A] = X[B, D] * W[D, A]        (computed with cuBLAS column-major trick)
  probs[B, A]  = softmax_rowwise(logits)
      ...
      d_gradW_(nullptr), d_tmp_(nullptr), cap_B_(0)
{
    ...
    std::vector<float> h_W(cfg_.state_dim * cfg_.num_actions);
    float scale = std::sqrt(2.0f / (cfg_.state_dim + cfg_.num_actions));
    for (auto& w : h_W) w = scale * (float(rand())/RAND_MAX - 0.5f);
...
void Policy::ensure_capacity(int B) {
    ...
    CUDA_OK(cudaMalloc(&d_probs_,  sizeof(float) * B * cfg_.num_actions));
    CUDA_OK(cudaMalloc(&d_gradW_,  sizeof(float) * cfg_.state_dim * cfg_.num_actions));
    CUDA_OK(cudaMalloc(&d_tmp_,    sizeof(float) * B));
}
                     ...
                     std::vector<float>& h_probs)
{
    ...
    // Column-major trick: compute with (m=A, n=B, k=D), A=W (lda=A), B=X (ldb=D), C=logits (ldc=A)
    const float alpha = 1.0f, beta = 0.0f;
    CUBLAS_OK(cublasSgemm(
        g_blas,
        CUBLAS_OP_N, CUBLAS_OP_N,
        /* m = */ cfg_.num_actions,   // A
        /* n = */ B,                  // B
        /* k = */ cfg_.state_dim,     // D
        &alpha,
...
void Policy::update(const float* h_X, const int* h_A, const float* h_Adv, int B) {
    ...
    // 1) Forward: X->logits->probs (device only)
    CUDA_OK(cudaMemcpy(d_X_, h_X, sizeof(float) * B * cfg_.state_dim, cudaMemcpyHostToDevice));
    ...
    CUBLAS_OK(cublasSgemm(
        ...
        /* k = */ cfg_.state_dim,     // D
        &alpha1,
        /* A = W */ d_W_,             /* lda = A */ cfg_.num_actions,
        /* B = X */ d_X_,             /* ldb = D */ cfg_.state_dim,
        &beta0,
        /* C = logits */ d_logits_,   /* ldc = A */ cfg_.num_actions));

    int t = 256, nb = (B + t - 1) / t;
    softmax_rowwise<<<nb, t>>>(d_logits_, d_probs_, B, cfg_.num_actions);
    CUDA_OK(cudaPeekAtLastError());
    ...
    // We use transA = T, transB = N.
    // For column-major, leading dimensions are the number of rows of the underlying (pre-transpose) matrices:
    //   A (underlying X) is BxD  -> lda = B
    //   B (dlogits/probs) is BxA -> ldb = B
    //   C (gradW) is D x A       -> ldc = D
    const float alpha = 1.0f / std::max(1, B);
    const float beta  = 0.0f;
    zero_grad();
    CUBLAS_OK(cublasSgemm(
        ...
        /* k = */ B,
        &alpha,
        /* A = X[B,D] */ d_X_,        /* lda = B */ B,                  // <-- critical fix
        /* B = dlogits[B,A]=d_probs_ */ d_probs_, /* ldb = B */ B,       // <-- critical fix
        &beta,
        /* C = gradW[D,A] */ d_gradW_,/* ldc = D */ cfg_.state_dim));    // <-- correct
